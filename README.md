...
| 1.5.1 | **Quick-Capture Single-Image + Clean Popup Start State.** Hover button label is hardened with a `safeText()` helper so it always renders `"Pin Affiliate"` and never the literal `"undefined"`. Quick-capture now uses **only** the exact image the user clicked — no full-page gallery scan, no random images. Direct popup-icon click no longer dumps a messy gallery; instead it shows a clean `"Start a Pin"` card with **Scan Page Images** / **Paste Amazon URL Manually** / **Open Options** buttons. Gallery only appears after the user picks Scan Page Images or clicks Change Image. Pinterest Content + Quality Score sections use progressive disclosure — they stay hidden until there's an image plus a caption or Amazon URL, so the popup never flashes a stale `97/100` next to author/admin/group metadata. New `looksLikeMetadata()` guard in `popup.js` recognises Facebook author/admin/group-expert pollution and blanks out the auto-generated title / description / hashtags so the user is told to paste or edit a real caption instead of seeing junk. Setup-warning card prompts to set the Associate tag when missing. All existing functionality preserved (queue, templates, quality, auto-fix, options, manual publish only). |
